...
- (UIView *)makeFrame:(CGRect)rect {
    return [[self makeOriginX:rect.origin.x Y:rect.origin.y] makeBounds:rect];
}

- (UIView *)makeBounds:(CGRect)rect {
    return [self makeSizeWidth:rect.size.width Height:rect.size.height];
}
...
- (UIView *)transformScale:(CGFloat)f {
    [self addAnimationCalculationAction:^(UIView *weakSelf) {
        JHKeyframeAnimation *transformAnimation = [weakSelf basicAnimationForKeyPath:@"transform"];
        CATransform3D transform = weakSelf.layer.transform;
